...
- **SHOULD:** maintain at least two DCs per domain per site with WAN connectivity for redundancy; keep FSMO roles on infrastructure with headroom for planned maintenance elsewhere. Where a programme spans multiple DCs across multiple domains/forests (e.g. a multi-forest OS upgrade programme), apply the change to one DC first — the lowest-impact candidate, typically a non-FSMO-holding DC in a site with redundant coverage — and validate fully before proceeding to the remaining DCs, rather than scheduling all DCs in the programme for the same maintenance window. This canary-first pattern, already standard practice in `workflows/linux-cis-hardening-lifecycle/WORKFLOW.md` and `workflows/vmware-esxi-vcenter-upgrade-lifecycle/WORKFLOW.md`, applies equally here: a version-specific or environment-specific issue caught on one DC is far cheaper to discover than the same issue surfacing simultaneously across a forest.
