...
## [Milestone 24] - 2026-07-01

### Changed — Platform Generalization for Multi-Client Consulting Use

The platform was originally built around one reference environment's real details (organization name, specific hostnames, specific site names, a specific vendor mix presented as fixed reality, one real production incident, and one client's specific ITSM tool). Moving this repository to personal/consulting use required removing every organization-identifying and single-client-specific assertion so the platform genuinely works for any engagement, not just the one it was originally built against. This was a cross-cutting pass, not a single-file change:

- **CLAUDE.md and README.md** — Role/mission statements rewritten around a vendor-neutral, multi-client consulting practice pursuing ISO/IEC 27001, PCI-DSS, and ITIL v4 outcomes for whichever client is currently engaged, rather than an internal platform for one organization. Quality bar extended with explicit vendor-neutrality and portability checks ("does this artifact still work if the client's tooling is completely different from the example used to illustrate it").
- **Nine agent files** — removed all "this estate"/"our estate" framing (which asserted a fixed, single-organization environment as platform fact) and replaced with engagement-neutral language requiring the specific vendor/engine/environment to be confirmed per client. Removed a personal name attribution from `knowledge/index.md`. Removed direct references to one organization's internal tooling (`zss-change-validator`) and audit history (a specific Veeam estate audit, a specific change record number).
- **`agents/network-architect/AGENT.md`, `workflows/network-core-switching-upgrade/WORKFLOW.md`, `examples/10g-network-migration-programme-charter/WALKTHROUGH.md`** — three real datacenter site names replaced with generic Site A/B/C; the "10G migration programme" reframed from a fixed active named initiative to an illustrative programme type (a network capacity upgrade, of which a 10G migration is one example); removed a vendor (Supermicro) that was listed as fixed inventory rather than a genuinely common network vendor.
- **`agents/database-engineer/AGENT.md`, `agents/openstack-architect/AGENT.md`** — removed a specific versioned artifact reference (`v7.3.7`) and an "active, named issue" framing that asserted a specific unresolved production issue as platform fact; reframed as common, well-understood issue classes instead.
- **`examples/sssd-ldap-firewall-rca-real-incident/` renamed to `examples/sssd-ldap-firewall-rca-case-study/`** — the platform's one real-incident example was explicitly labeled "an actual incident from this estate's real work" with real hostnames (`prd-apexia`, `prd-ability`) and a real change record number (`C-082327`). Rewritten as an anonymized composite case study: same genuinely valuable diagnostic pattern (stateful firewall silently dropping idle LDAP connections), no identifying detail. The old path is preserved in `tests/validate-repo.sh`'s deferred-reference allow-list and `docs/cross-reference-audit-milestone-13.md`'s table so `CHANGELOG.md`'s historical mentions of the old name don't get flagged as broken — CHANGELOG history is preserved as-written, not retroactively rewritten, per this platform's own established discipline.
- **iTop assumption removed platform-wide** — every workflow's Prerequisites and Acceptance Criteria previously assumed iTop specifically as *the* ITSM/CMDB platform; now framed as "the client's ITSM/CMDB platform (e.g. iTop, ServiceNow, or equivalent)," confirmed per engagement.
- **`tools/skill-source/SKILL.md`** — renamed from `enterprise-engineering-platform` to `certification-consulting-platform`, description and body rewritten around certification-consulting use, the same fixed-vendor-list assertion in its operating rules fixed to match `knowledge/index.md`'s corrected framing. Rebuilt and repackaged (`certification-consulting-platform.skill`) from the fully genericized source — **anyone with the old `enterprise-engineering-platform.skill` installed should replace it with this one**, since the name itself changed.
- Minor: four "this estate" references inside `tests/agent-behavior/` simulated agent-response transcripts also genericized, for completeness, even though these are test artifacts rather than live guidance.

### Verification
- `tests/validate-repo.sh` re-run after every batch of changes throughout this pass (not just once at the end), consistent with the discipline established in prior milestones: **17 passed, 0 failed**, final state.
- Skill rebuilt (`tools/build-skill.sh`) and repackaged from the fully genericized source; validated cleanly through the skill-creator tooling before packaging.
- Full-repo sweep for `ZSS`, `Zimnat`, `Tonderai Chiruka` (outside `LICENSE`, which legitimately retains it as the platform's actual IP-holder/consultant copyright), `zss-change-validator`, the three real site names, and `this estate`/`our estate`: **zero remaining matches outside `CHANGELOG.md`'s preserved historical record.**
