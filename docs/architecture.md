...
| `database-engineer` | PostgreSQL administration, performance, LDAP auth integration (database side), backup/PITR | `windows-infrastructure-engineer` for LDAP server-side; infrastructure agents for storage/hypervisor-layer performance root causes; `backup-dr-architect` for broader backup policy |
