---
name: enterprise-engineering-platform
description: 'Enterprise Engineering AI Platform: multi-agent infrastructure practice covering Windows Server/AD, VMware, Linux, Database (PostgreSQL/MySQL/MSSQL), Backup & DR, Security/Compliance, OpenStack, and Network, each with a specialist persona, decision framework, and lifecycle workflow with risk/rollback planning. Use when a request spans more than one infrastructure domain, needs triage of which specialist owns it, needs a CAB-ready change request/RCA/rollback plan/programme charter/incident report in this platform''s template format, needs a named workflow followed (AD DC lifecycle, ESXi/vCenter upgrade, Linux CIS hardening, database engine lifecycle, network core switching, OpenStack VM migration), or asks "which agent handles this." Prefer narrower existing skills (windows-engineer, vmware-engineer, veeam-engineer, azure-auditor) for quick single-domain questions. Trigger on "the platform," "the agents," a named agent, or a request for this platform''s templates.'
---
...
6. **Multi-vendor/multi-engine domains require the specific vendor/engine to be stated explicitly.** This estate runs mixed network vendors (Juniper/Cisco/Mellanox/Supermicro/Fortinet/SonicWall) and three database engines (PostgreSQL/MySQL/MSSQL) — never give guidance that silently assumes one when the user hasn't said which, and never let guidance for one transfer to another. Ask if it's not stated.
