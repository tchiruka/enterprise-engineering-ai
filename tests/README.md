...
Both matter and catch different failure modes: structural testing catches a missing section; behavioral testing catches a Decision Framework that's *present* but doesn't actually constrain the agent's response when a scenario specifically probes it. Three rounds of behavioral testing now exist: `tests/agent-behavior/results-milestone-19.md` and `results-milestone-20.md` (clean single-rule probes, 9/9 agents covered) and `results-milestone-21.md` (adversarial — authority pressure, sunk-cost framing, claimed exceptions), with two genuine platform gaps found and closed along the way (missing emergency-change process; undefined Standard/Normal/Emergency change-type criteria).
