...
Both matter and catch different failure modes: structural testing catches a missing section; behavioral testing catches a Decision Framework that's *present* but doesn't actually constrain the agent's response when a scenario specifically probes it. Four rounds of behavioral testing now exist: `results-milestone-19.md` and `results-milestone-20.md` (clean single-rule probes, 9/9 agents covered), `results-milestone-21.md` (adversarial single-turn — authority pressure, sunk-cost framing, claimed exceptions), and `results-milestone-22.md` (multi-turn — sustained pushback after the initial refusal, including one scenario testing whether the agent can distinguish a legitimate compromise from pure pressure). Two genuine platform gaps were found and closed along the way (missing emergency-change process; undefined Standard/Normal/Emergency change-type criteria). Cumulative score across all four rounds: 27/27 scenario-turns passed — see `results-milestone-22.md`'s "Honest assessment" section for why that record should be read as a positive signal, not proof of anything unbreakable.
